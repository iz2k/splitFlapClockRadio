...
def create_install_script(component_name, component_description, wheel_name):
    ...
    install_script.addLine('sudo rm -rf /usr/share/executor/' + component_name)
    install_script.addLine('sudo mkdir /usr/share/executor/' + component_name)
    install_script.addLine('echo " -> Create virtual environment"')
    install_script.addLine('sudo python3 -m venv /usr/share/executor/' + component_name + '/venv')
    install_script.addLine('sudo cp version.txt /usr/share/executor/' + component_name + '/')
    install_script.addLine('echo " -> Install component"')
    install_script.addLine('sudo /usr/share/executor/' + component_name + '/venv/bin/pip install ' + wheel_name)
    install_script.addLine('echo " -> Create executable script"')
    install_script.addLine('sudo touch /usr/share/executor/' + component_name + '/' + component_name + '.sh')
    install_script.addLine('echo "/usr/share/executor/' + component_name + '/venv/bin/python -m ' + component_name + ' \\"\$@\\"" | sudo tee -a /usr/share/executor/' + component_name + '/' + component_name + '.sh | grep donotshowanythinginbash')
    install_script.addLine('sudo chmod +x /usr/share/executor/' + component_name + '/' + component_name + '.sh')
    install_script.addLine('echo " -> Create symbolic link"')
    install_script.addLine('sudo ln -f -s /usr/share/executor/' + component_name + '/' + component_name + '.sh /usr/bin/' + component_name)
