...
class UserInterface:
    ...
    def vol_rotary_callback(self, direction):
        ...
        if (self.app.mainControlTh.queue != None):
            self.app.mainControlTh.queue.put(['volume_rotary', direction])

    def vol_sw_short(self):
        #print("[UI] VOL_SW SHORT")
        if (self.app.mainControlTh.queue != None):
            self.app.mainControlTh.queue.put(['volume_switch', 'short'])

    def vol_sw_long(self):
        #print("[UI] VOL_SW LONG")
        if (self.app.mainControlTh.queue != None):
            self.app.mainControlTh.queue.put(['volume_switch', 'long'])

    def ctrl_rotary_callback(self, direction):
        #print("[UI] CTRL_ROTARY:", direction)
        if (self.app.mainControlTh.queue != None):
            self.app.mainControlTh.queue.put(['control_rotary', direction])

    def ctrl_sw_short(self):
        #print("[UI] CTRL_SW SHORT")
        if (self.app.mainControlTh.queue != None):
            self.app.mainControlTh.queue.put(['control_switch', 'short'])

    def ctrl_sw_long(self):
        #print("[UI] CTRL_SW LONG")
        if (self.app.mainControlTh.queue != None):
            self.app.mainControlTh.queue.put(['control_switch', 'long'])
